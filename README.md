...
A customer and vehicle are registered by staff or through the public form.
A Service Advisor creates a service booking.
The system checks the requested time against existing bookings for the same vehicle.
A job card is created from the booking.
Active mechanics and available parts are assigned.
The assigned Mechanic updates the job to In Progress.
Diagnosis and completed work are recorded.
When the job becomes Completed, stock is deducted once inside a transaction.
An invoice is generated from labor and parts costs.
The invoice can be marked Paid.
An authorized user can generate an AI Service Summary for the completed job.
---
Architecture
...
Workflow
The job must have a Completed status.
The user clicks Generate AI Summary.
The controller authorizes access to the job card.
`AIServiceSummaryService` loads the required customer, vehicle, mechanics, and parts information.
The service creates a focused prompt.
Laravel sends a server-side request to the OpenAI Responses API.
The service validates and returns the generated text.
The controller stores:
`ai_summary`
`ai_summary_generated_at`
The React page displays the saved summary and provides a regeneration action.
...
```
Use the secure fix:
Download an official CA certificate bundle.
Store it in a stable local path, for example:
```text
   D:\KALINDU\cacert.pem
   ```
Identify the active `php.ini`:
```powershell
   php --ini
   ```
Set these values in the active `php.ini`:
```ini
   curl.cainfo = "D:\KALINDU\cacert.pem"
   openssl.cafile = "D:\KALINDU\cacert.pem"
   ```
Restart Laravel, Vite, and Tinker.
...
Lock or safely load the required part records.
Validate available quantities.
Deduct each assigned quantity.
...
Test Steps
Log in as Admin.
Confirm the dashboard displays live database values.
Create Nimal Fernando.
Create the Toyota Corolla and connect it to Nimal.
Create the sample mechanics.
Create the sample parts and confirm the brake-pad set is shown as low stock.
Create a booking for `WP CAB-2345`.
Attempt an overlapping booking for the same vehicle and confirm it is rejected.
Create a job card from the valid booking.
Assign Dilan Fernando.
Assign one Engine Oil and one Toyota Oil Filter.
Set labor cost to LKR 8,000.
Log in as the Mechanic and confirm only the assigned job appears.
Change the job to In Progress.
Enter diagnosis and work-completed details.
Mark the job Completed.
Confirm Engine Oil stock changes from 12 to 11.
Confirm Oil Filter stock changes from 8 to 7.
...
Confirm the dashboard revenue includes LKR 16,750.
Generate and display the AI Service Summary.
Open Admin Users and Roles and demonstrate role assignment.
---
Troubleshooting
...
---
Future Improvements
Add feature tests for all critical business rules.
Add an activity log and audit trail.
Add PDF invoice export.
Add Excel reports.
Add booking and invoice email notifications.
Add a calendar booking view.
Add Docker and CI configuration.
Add production monitoring and scheduled backups.
---
Git Workflow
